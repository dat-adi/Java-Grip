/**
 *
 * A program to solve the water jug problem.
 * Take a three liter jug and a four liter jug.
 * Balance the water in the jugs such that the
 * four liter jug ends up having 2 liters in the end.
 *
     ...
     private static void getPathIfPossible(int jug1, int jug2, int target){
        ...
        while(!queue.isEmpty()){
            ...
            if(curr.j1 == target || curr.j2 == target){
                ...
                System.out.println("[!] Path of states of jugs followed");
                for(int i = 0; i < n; i++)
                    System.out.println("[=>] " + curr.path.get(i).j1 
                        + ", " +curr.path.get(i).j2);
                return;
            }
